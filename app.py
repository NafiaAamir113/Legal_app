...
# UI Components
st.title("⚖️ Legal Retrieval-Augmented Generation (RAG) System")
query = st.text_input("🔍 Enter your legal question:")
...
if query:
    ...
    with st.spinner("🔎 Searching..."):
        ...
        # Extract relevant case texts
        legal_cases = [match["metadata"]["text"] for match in search_results["matches"]]

        # Rerank cases
        rerank_scores = reranker.predict([(query, case) for case in legal_cases])
        ranked_cases = sorted(zip(legal_cases, rerank_scores), key=lambda x: x[1], reverse=True)

        # Select up to 5 top cases
        num_cases = min(len(ranked_cases), 5)
        context_text = "\n\n".join([r[0] for r in ranked_cases[:num_cases]])

        # **Strict Prompt to Prevent Hallucination & Extra Text**
        prompt = f"""
        You are a legal assistant. Generate a **formal legal report** strictly using the provided case details. 
        
        **Rules:**
        - Only use the information in the provided cases.
        - Do NOT mention "retrieved documents" or similar phrases.
        - Ensure the report is clear, professional, and usable by lawyers.
        - Avoid unnecessary disclaimers—just provide the legal analysis.
